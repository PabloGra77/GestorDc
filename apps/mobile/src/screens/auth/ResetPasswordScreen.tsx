...
import { confirmPasswordReset, getAuthErrorMessage } from '../../services/auth.service';
...
export function ResetPasswordScreen({ navigation, route }: Props) {
  ...
  async function handleSubmit() {
    ...
    if (!token.trim()) {
      setError('Pega el token que recibiste en tu correo.');
      return;
    }
    ...
    setCargando(true);
    try {
      const res = await confirmPasswordReset({ token: token.trim(), newPassword });
      setMensaje(res.message || 'Contraseña restablecida correctamente.');
      setNewPassword('');
      setConfirmPassword('');
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.brand}>
            ...
            <Text style={styles.appSubtitle}>Define una contraseña segura para tu cuenta.</Text>
          </View>
