...
type AdminModule = 'Usuarios' | 'Roles' | 'Usuarios en linea';
	...
	return (
		...
		>
			{activeSection === 'Inicio' ? (
				<section className="card-surface start-radicado-panel mail-layout">
					...
					<div className="start-radicado-grid mail-panel-grid">
						<section className="card-surface mail-compose-launcher" aria-label="Compositor de radicado">
							<div className="mail-panel-head">
								<h4>Nueva solicitud</h4>
							</div>
							<div className="mail-compose-toolbar">
								{canRealizarSolicitudes ? (
									<button
										type="button"
										className="radicado-action-button compose-trigger"
										onClick={() => {
											setRadicadoError('');
											setRadicadoMessage('');
											setIsComposeRadicadoOpen(true);
										}}
									>
										Realizar nueva solicitud
									</button>
								) : (
									<p className="admin-help-text">No tienes permiso para crear solicitudes.</p>
								)}
							</div>
						</section>

						<form className="admin-form card-surface mail-tracking-panel mail-tracking-compact" onSubmit={handleVerifyRadicado}>
							<div className="mail-panel-head">
								<h4>Seguimiento de radicado</h4>
								<span>Consulta por numero o referencia</span>
							</div>
							{canVerificarRadicados ? (
								<div className="mail-tracking-inline">
									<input
										type="text"
									...
									/>
									<button type="submit" className="radicado-action-button mail-tracking-search-button">
										Buscar
									</button>
								</div>
							) : (
								<p className="admin-help-text">No tienes permiso para verificar radicados.</p>
							)}

							{verificacionRadicado ? (
								<div className="radicado-result card-surface">
									<strong>Radicado verificado</strong>
									<span>Numero: {verificacionRadicado.numero}</span>
									<span>Referencia: {verificacionRadicado.referencia}</span>
									<span>Estado: {verificacionRadicado.estado}</span>
								</div>
							) : null}
							...
							{isVerificadoSinResultado ? (
								<div className="radicado-result card-surface">
									...
									<span>No existe ese numero o referencia en base de datos.</span>
								</div>
							) : null}
						</form>
					</div>
			...
			{activeSection === 'Panel administrador' && esAdmin ? (
				<section className="card-surface admin-panel admin-forms-panel">
					...
					<div className="admin-module-nav" role="tablist" aria-label="Módulos administrativos">
						{(['Usuarios', 'Roles', 'Usuarios en linea'] as AdminModule[]).map((module) => (
							<button
								key={module}
								type="button"
