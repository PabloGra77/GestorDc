...
export async function createCuentaCobroOpsSolicitud(
  ...
  const response = await api.post<CreateCuentaCobroOpsResponse>('/radicados/cuentas-cobro-ops/solicitud', payload);
  return response.data;
}

export async function verifyCuentaCobroOps(numero: string, cc: string): Promise<VerifyCuentaCobroOpsResponse> {
  const response = await api.get<VerifyCuentaCobroOpsResponse>('/radicados/cuentas-cobro-ops/verificar', {
    params: { numero, cc },
  });
  return response.data;
}
...
export async function submitCuentaCobroOpsDocumentos(
  ...
  const response = await api.post<{ ok: boolean; message: string; numero: string; estado: string }>(
    '/radicados/cuentas-cobro-ops/documentos',
    payload,
  );
  return response.data;
}
