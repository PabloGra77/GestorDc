...
function NavIcon({ item }: { item: string }) {
	...
	if (item === 'Portal de Pagos') return (
		<svg width={s} height={s} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
			<path d="M4 6h16a1 1 0 0 1 1 1v10a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V7a1 1 0 0 1 1-1zm0 3v6h16V9H4zm2 2h4v2H6v-2z" />
		</svg>
	);
...
export function Sidebar({ esAdmin, mostrarPortalPagos, activeSection, onSelectSection, onNavigateRadicaciones, currentUser, rol, onLogout }: SidebarProps) {
	...
	const menuItems = [
		'Inicio',
		'Radicaciones',
		...(esAdmin || mostrarPortalPagos ? ['Portal de Pagos'] : []),
		...(esAdmin ? ['Panel administrador'] : []),
		'Mi perfil',
	];
			...
			{menuItems.map((item) => (
				...
				>
					...
					<span className="admin-bottom-nav-label">
						{item === 'Panel administrador' ? 'Admin' : item === 'Portal de Pagos' ? 'Pagos' : item}
					</span>
				</button>
			))}
		</nav>,
