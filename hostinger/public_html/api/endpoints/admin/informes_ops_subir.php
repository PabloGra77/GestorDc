...
$esPad     = fn(string $s): bool => str_ends_with(mb_strtoupper($s), ' PAD') || mb_strtoupper($s) === 'PAD';
...
$stmt->execute([':nom' => $nombre, ':pi' => $periodoInicio ?: null, ':pf' => $periodoFin ?: null, ':uid' => $uid ?: null]);
...
for ($i = 1, $n = count($lineas); $i < $n; $i++) {
    ...
    // Para servicios PAD: el numero_sesiones viene del CSV; para SM: siempre 1
    $ns = 1;
    if ($sv && $esPad($sv)) {
    ...
    $batch[] = [
        ...
        'reg' => isset($cols['regional'])         ? mb_substr(trim((string)($f[$cols['regional']]    ?? '')), 0, 200) : null,
        'est' => isset($cols['establecimiento'])  ? mb_substr(trim((string)($f[$cols['establecimiento']] ?? '')), 0, 200) : null,
...
// Actualizar total_filas (suma de sesiones, no cantidad de filas)
